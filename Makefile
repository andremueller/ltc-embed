...
ARGS = $(if $(FPS),--fps $(FPS)) --suffix $(SUFFIX) --duration $(DURATION) --offset $(OFFSET) $(if $(DRIFT_AUTO),--drift-auto) $(if $(OVERWRITE),--overwrite) --verbose
...
run: venv ## Process video files (INPUT_DIR, FPS, SUFFIX, DURATION, OFFSET, DRIFT_AUTO, OVERWRITE)
